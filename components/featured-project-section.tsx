...
export function FeaturedProjectSection({ language }: FeaturedProjectSectionProps) {
  ...
  return (
    <motion.section
      ...
      className="relative xl:grid xl:grid-cols-[1.15fr_0.85fr] xl:items-stretch"
    >
      {/* ---------- Left: content (beige page background shows through) ---------- */}
      <div className="px-6 py-24 sm:px-12 lg:py-32 xl:pl-28 xl:pr-16">
        ...
        <div className="mt-10 space-y-8">
          <div className="flex flex-wrap gap-3">
            {t.metrics.map((metric) => (
              <span
                key={metric}
                className="inline-flex items-center gap-2 rounded-full border border-foreground/10 bg-foreground/[0.03] px-4 py-2 text-xs font-medium uppercase tracking-[0.24em] text-foreground/80"
              >
                {metric}
              </span>
            ))}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="rounded-[1.75rem] border border-foreground/10 bg-foreground/[0.03] p-6">
      ...
      {/* ---------- Right: full-bleed dark mockup panel (desktop only) ---------- */}
      <div className="relative hidden xl:block">
        <img
          src="/images/featuredproject-bg.png"
          alt="inFLOW product mockup"
          className="absolute inset-0 h-full w-full object-cover object-right"
        />
      </div>

      {/* ---------- Mobile / tablet: only the inflow.png card ---------- */}
      <div className="xl:hidden px-6 pb-24 sm:px-12">
        <div className="relative w-full overflow-hidden rounded-[2rem] border border-foreground/10 bg-foreground/[0.03] shadow-2xl shadow-black/10">
          <img
